/**
 * IsoRendererNeo
 * Special optimized renderer for 2.5D Isometric volumetric views of multi-chunk grids for Neo.
 */
export class IsoRendererNeo {
    ...
    public renderMultiChunkVolume(
        ...
        cols: number,
        rows: number,
        options: { densityFaceIndex: number, obstacleFaceIndex?: number, lodStep?: number }
    ) {
        const { densityFaceIndex: dfi, obstacleFaceIndex: ofi } = options;
        const step = options.lodStep || 2; // Level of Detail to reduce draw calls
        ...
        for (let gy = 0; gy < rows; gy++) {
            for (let gx = 0; gx < cols; gx++) {
                ...
                for (let ly = 1; ly < ny - 1; ly += step) {
                    for (let lx = 1; lx < nx - 1; lx += step) {
                        const idx = ly * nx + lx;
                        ...
                        this.ctx.fillRect(x, y - h, scale * step + 0.5, h || scale);
                    }
                }
            }
        }
