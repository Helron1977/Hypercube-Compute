...
async function bootSDF() {
    ...
    const resManifest = await fetch('./showcase-sdf.json?v=' + Date.now());
    ...
    const criteriaIndices = Object.keys(MAPPINGS).map(key => ({
        key,
        xFace: engine.parityManager.getFaceIndices(`sdf_${key}_x`).read,
        yFace: engine.parityManager.getFaceIndices(`sdf_${key}_y`).read,
