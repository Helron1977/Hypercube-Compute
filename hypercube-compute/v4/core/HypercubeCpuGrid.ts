...
export class HypercubeCpuGrid {
    ...
    constructor(
        ...
        for (let y = 0; y < rows; y++) {
            ...
            for (let x = 0; x < cols; x++) {
                const cube = new HypercubeChunk(x, y, this.nx, this.ny, this.nz, masterBuffer, finalNumFaces, this.padding);
    ...
    static async create(
        ...
        const grid = new HypercubeCpuGrid(cols, rows, resolution, masterBuffer, engineFactory, numFaces, isPeriodic, useWorkers, padding);
    ...
    /**
     * Pushes all CPU data (obstacles, initial states) from all chunks to simulation memory.
     */
    public push() {
        // GPU removal: this is now just a placeholder or could be used for other sync
    }
