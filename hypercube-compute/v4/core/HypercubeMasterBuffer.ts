export class HypercubeMasterBuffer {
     ...
     */
    allocateCube(nx: number, ny: number, nz: number = 1, numFaces: number = 6, padding: number = 0): { offset: number, stride: number } {
        const ALIGNMENT = 256;

        const pnx = nx + padding * 2;
        const pny = ny + padding * 2;
        const pnz = nz > 1 ? nz + padding * 2 : 1;
        ...
        const cellsPerFace = pnx * pny * pnz;
        const bytesPerFaceRaw = cellsPerFace * 4 * 2; // 8 bytes (2 floats for Ping-Pong) per cell
